...
export function RecruitmentCard({ data }: { data: RecruitmentList }) {
  return (
    ...
    >
      {data.thumbnail ? (
        <Image src={data.thumbnail} alt={`${data.title} thumbnail`} width={1920} height={1020} quality={100} className="h-auto w-full rounded-md object-cover sm:rounded-[28px]" unoptimized />
      ) : (

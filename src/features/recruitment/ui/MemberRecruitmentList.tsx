...
import { MemberAddRecruitmentButton, DeleteRecruitmentButton, MemberEditRecruitmentButton, RecruitmentStatusGrid } from './RecruitmentListButton';
import { useRecruitmentByMember } from 'entities/recruitment';
...
export const MemberRecruitmentList = () => {
  const { data } = useRecruitmentByMember();

  return (
    <div className="flex flex-col items-center gap-6">
      <div className="flex w-full flex-row justify-end">
        <MemberAddRecruitmentButton />
      </div>
      <Table>
        <TableHeader>
...
const Item = ({ data }: { data: ReturnType<typeof useRecruitmentByMember>['data'][number] }) => {
  return (
    <tr className="group transition-colors hover:bg-slate-50/50">
      ...
      <td className="px-6 py-5 text-right">
        <div className="flex justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
          <MemberEditRecruitmentButton id={data.id} />

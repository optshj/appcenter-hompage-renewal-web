...
import { Save, CheckCircle2, Image as ImageIcon, Plus } from 'lucide-react';
          ...
          <div className="flex flex-col gap-6 lg:flex-row">
            <div className="flex flex-1 flex-col gap-4">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-700">모집 설명(Body)</label>
                <textarea
                  value={form.body}
                  onChange={(e) => setForm({ ...form, body: e.target.value })}
                  rows={4}
                  className="w-full resize-none rounded-lg border border-slate-200 px-3 py-2 text-sm transition-all outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  placeholder="간단한 모집 문구를 입력하세요"
                />
              </div>
            </div>
